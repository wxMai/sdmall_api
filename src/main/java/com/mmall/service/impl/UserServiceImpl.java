...
public class UserServiceImpl implements IUserService
{
    ...
    public ServerResponse<User> updateInformationAndPassword(User user)
    {
        //username是不能被更新的
        //email也要进行一个校验,校验新的email是不是已经存在,并且存在的email如果相同的话,不能是我们当前的这个用户的.
        int resultCount = userMapper.checkEmailByUserId(user.getEmail(), user.getId());
        ...
        if (pwd.length() > 0) {
            updateUser.setPassword(MD5Util.MD5EncodeUtf8(pwd));
        }
    ...
    public ServerResponse<UserVo> getUserInfoAndShipping(Integer userId)
    {
        User user = userMapper.selectByPrimaryKey(userId);
        if (user == null) {
            return ServerResponse.createByErrorMessage("找不到当前用户");
        ...
        user.setPassword(org.apache.commons.lang3.StringUtils.EMPTY);
        UserVo res = assembleShipping(user);
        return ServerResponse.createBySuccess(res);

    }

    private UserVo assembleShipping(User user)
    {
        Integer userId = user.getId();

        UserVo userVo = new UserVo();
        userVo.setValueByUser(user);

        Shipping shipping = shippingMapper.selectOneByUserId(userId);
        if (shipping != null) {
            userVo.setShipping(shipping);
        }
